...
idea is that the \emph{order of events} is fundamental to the
mathematical story arc: when playing out any specific example story in
that arc, the beginning of the story affects not only the later events,
but even the very terms with which the later events will be described.
For example, in the story arc of conditional probability, one may say
``now if the set \(P\) that we are asked to condition on happens to have
measure zero, we must stop; but assuming that's not the case then the
result will be a new probability measure.'\,' Here the story teller is
saying that no terms will describe what happens if \(P\) has measure
zero, whereas otherwise the terms of standard probability will apply.
...
whole. As an illustration of this, we show how every polynomial
universe, regarded as a polynomial pseudomonad with additional
structure, gives rise to self-distributive law \(u\tri u\to u\tri u\),
which witnesses the usual distributive law of dependent products over
dependent sums.
...
We begin with a quick recap of the basics of HoTT and polynomial
functors.
...
\NormalTok{isGrpd }\OtherTok{:} \OtherTok{∀} \OtherTok{\{}\NormalTok{ℓ}\OtherTok{\}} \OtherTok{→}\NormalTok{ Type ℓ }\OtherTok{→}\NormalTok{ Type ℓ}
\NormalTok{isGrpd A }\OtherTok{=} \OtherTok{(}\NormalTok{a b }\OtherTok{:}\NormalTok{ A}\OtherTok{)} \OtherTok{→}\NormalTok{ isSet }\OtherTok{(}\NormalTok{a ≡ b}\OtherTok{)}

\NormalTok{isEquiv }\OtherTok{:} \OtherTok{∀} \OtherTok{\{}\NormalTok{ℓ κ}\OtherTok{\}} \OtherTok{\{}\NormalTok{A }\OtherTok{:}\NormalTok{ Type ℓ}\OtherTok{\}} \OtherTok{\{}\NormalTok{B }\OtherTok{:}\NormalTok{ Type κ}\OtherTok{\}}
          \OtherTok{→} \OtherTok{(}\NormalTok{A }\OtherTok{→}\NormalTok{ B}\OtherTok{)} \OtherTok{→} \DataTypeTok{Set} \OtherTok{(}\NormalTok{ℓ ⊔ κ}\OtherTok{)}
\NormalTok{isEquiv }\OtherTok{\{}\NormalTok{A }\OtherTok{=}\NormalTok{ A}\OtherTok{\}} \OtherTok{\{}\NormalTok{B }\OtherTok{=}\NormalTok{ B}\OtherTok{\}}\NormalTok{ f }\OtherTok{=} \OtherTok{(}\NormalTok{b }\OtherTok{:}\NormalTok{ B}\OtherTok{)} \OtherTok{→}\NormalTok{ isContr }\OtherTok{(}\NormalTok{Σ A }\OtherTok{(λ}\NormalTok{ a }\OtherTok{→}\NormalTok{ f a ≡ b}\OtherTok{))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{PrePoly }\OtherTok{:} \OtherTok{(}\NormalTok{ℓ κ }\OtherTok{:}\NormalTok{ Level}\OtherTok{)} \OtherTok{→}\NormalTok{ Type }\OtherTok{((}\NormalTok{lsuc ℓ}\OtherTok{)}\NormalTok{ ⊔ }\OtherTok{(}\NormalTok{lsuc κ}\OtherTok{))}
\NormalTok{PrePoly ℓ κ }\OtherTok{=}\NormalTok{ Σ }\OtherTok{(}\DataTypeTok{Set}\NormalTok{ ℓ}\OtherTok{)} \OtherTok{(λ}\NormalTok{ A }\OtherTok{→}\NormalTok{ A }\OtherTok{→} \DataTypeTok{Set}\NormalTok{ κ}\OtherTok{)}
\end{Highlighting}
\end{Shaded}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Lens }\OtherTok{:} \OtherTok{∀} \OtherTok{\{}\NormalTok{ℓ ℓ\textquotesingle{} κ κ\textquotesingle{}}\OtherTok{\}} \OtherTok{→}\NormalTok{ PrePoly ℓ κ }\OtherTok{→}\NormalTok{ PrePoly ℓ\textquotesingle{} κ\textquotesingle{} }\OtherTok{→}\NormalTok{ Type }\OtherTok{(}\NormalTok{ℓ ⊔ ℓ\textquotesingle{} ⊔ κ ⊔ κ\textquotesingle{}}\OtherTok{)}
\NormalTok{Lens }\OtherTok{(}\NormalTok{A , B}\OtherTok{)} \OtherTok{(}\NormalTok{C , D}\OtherTok{)} \OtherTok{=}\NormalTok{ Σ }\OtherTok{(}\NormalTok{A }\OtherTok{→}\NormalTok{ C}\OtherTok{)} \OtherTok{(λ}\NormalTok{ f }\OtherTok{→} \OtherTok{(}\NormalTok{a }\OtherTok{:}\NormalTok{ A}\OtherTok{)} \OtherTok{→}\NormalTok{ D }\OtherTok{(}\NormalTok{f a}\OtherTok{)} \OtherTok{→}\NormalTok{ B a}\OtherTok{)}
\end{Highlighting}
\end{Shaded}

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{record}\NormalTok{ Poly }\OtherTok{\{}\NormalTok{ℓ κ}\OtherTok{\}} \OtherTok{:}\NormalTok{ Type }\OtherTok{((}\NormalTok{lsuc ℓ}\OtherTok{)}\NormalTok{ ⊔ }\OtherTok{(}\NormalTok{lsuc κ}\OtherTok{))} \KeywordTok{where}
    \KeywordTok{constructor}\NormalTok{ poly}
    \KeywordTok{field}
\NormalTok{        Pos }\OtherTok{:}\NormalTok{ Type ℓ}
\NormalTok{        Dir }\OtherTok{:}\NormalTok{ Pos }\OtherTok{→}\NormalTok{ Type κ}
\NormalTok{        posIsGrpd }\OtherTok{:}\NormalTok{ isGrpd Pos}
\NormalTok{        dirIsSet }\OtherTok{:} \OtherTok{(}\NormalTok{p }\OtherTok{:}\NormalTok{ Pos}\OtherTok{)} \OtherTok{→}\NormalTok{ isSet }\OtherTok{(}\NormalTok{Dir p}\OtherTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{idToEquiv }\OtherTok{:} \OtherTok{∀} \OtherTok{\{}\NormalTok{ℓ κ}\OtherTok{\}} \OtherTok{(}\NormalTok{p }\OtherTok{:}\NormalTok{ PrePoly ℓ κ}\OtherTok{)} \OtherTok{(}\NormalTok{a b }\OtherTok{:}\NormalTok{ fst p}\OtherTok{)}
...
\NormalTok{idToEquiv p a b refl }\OtherTok{=} \OtherTok{(λ}\NormalTok{ x }\OtherTok{→}\NormalTok{ x}\OtherTok{)}\NormalTok{ , }\OtherTok{(λ}\NormalTok{ y }\OtherTok{→} \OtherTok{(}\NormalTok{y , refl}\OtherTok{)}\NormalTok{ , }\OtherTok{(λ} \OtherTok{\{} \OtherTok{(}\NormalTok{y\textquotesingle{} , refl}\OtherTok{)} \OtherTok{→}\NormalTok{ refl }\OtherTok{\}))}

\NormalTok{isUnivalent }\OtherTok{:} \OtherTok{∀} \OtherTok{\{}\NormalTok{ℓ κ}\OtherTok{\}} \OtherTok{→}\NormalTok{ PrePoly ℓ κ }\OtherTok{→}\NormalTok{ Type }\OtherTok{(}\NormalTok{ℓ ⊔ κ}\OtherTok{)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{isVertical }\OtherTok{:} \OtherTok{∀} \OtherTok{\{}\NormalTok{ℓ ℓ\textquotesingle{} κ κ\textquotesingle{}}\OtherTok{\}} \OtherTok{\{}\NormalTok{p }\OtherTok{:}\NormalTok{ PrePoly ℓ κ}\OtherTok{\}} \OtherTok{\{}\NormalTok{q }\OtherTok{:}\NormalTok{ PrePoly ℓ\textquotesingle{} κ\textquotesingle{}}\OtherTok{\}}
             \OtherTok{→}\NormalTok{ Lens p q }\OtherTok{→} \DataTypeTok{Set} \OtherTok{(}\NormalTok{ℓ ⊔ ℓ\textquotesingle{}}\OtherTok{)}
\NormalTok{isVertical }\OtherTok{(}\NormalTok{f , g}\OtherTok{)} \OtherTok{=}\NormalTok{ isEquiv f}

\NormalTok{isCartesian }\OtherTok{:} \OtherTok{∀} \OtherTok{\{}\NormalTok{ℓ ℓ\textquotesingle{} κ κ\textquotesingle{}}\OtherTok{\}} \OtherTok{\{}\NormalTok{p }\OtherTok{:}\NormalTok{ PrePoly ℓ κ}\OtherTok{\}} \OtherTok{\{}\NormalTok{q }\OtherTok{:}\NormalTok{ PrePoly ℓ\textquotesingle{} κ\textquotesingle{}}\OtherTok{\}}
             \OtherTok{→}\NormalTok{ Lens p q }\OtherTok{→} \DataTypeTok{Set} \OtherTok{(}\NormalTok{ℓ ⊔ κ ⊔ κ\textquotesingle{}}\OtherTok{)}
\NormalTok{isCartesian }\OtherTok{\{}\NormalTok{p }\OtherTok{=} \OtherTok{(}\NormalTok{A , B}\OtherTok{)\}} \OtherTok{(}\NormalTok{f , g}\OtherTok{)} \OtherTok{=} \OtherTok{(}\NormalTok{a }\OtherTok{:}\NormalTok{ A}\OtherTok{)} \OtherTok{→}\NormalTok{ isEquiv }\OtherTok{(}\NormalTok{g a}\OtherTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\OtherTok{\_}\NormalTok{◃}\OtherTok{\_} \OtherTok{:} \OtherTok{∀} \OtherTok{\{}\NormalTok{ℓ ℓ\textquotesingle{} κ κ\textquotesingle{}}\OtherTok{\}} \OtherTok{→}\NormalTok{ PrePoly ℓ κ }\OtherTok{→}\NormalTok{ PrePoly ℓ\textquotesingle{} κ\textquotesingle{} }\OtherTok{→}\NormalTok{ PrePoly }\OtherTok{(}\NormalTok{ℓ ⊔ κ ⊔ ℓ\textquotesingle{}}\OtherTok{)} \OtherTok{(}\NormalTok{κ ⊔ κ\textquotesingle{}}\OtherTok{)}
...
\OtherTok{\_}\NormalTok{⊗}\OtherTok{\_} \OtherTok{:} \OtherTok{∀} \OtherTok{\{}\NormalTok{ℓ ℓ\textquotesingle{} κ κ\textquotesingle{}}\OtherTok{\}} \OtherTok{→}\NormalTok{ PrePoly ℓ κ }\OtherTok{→}\NormalTok{ PrePoly ℓ\textquotesingle{} κ\textquotesingle{} }\OtherTok{→}\NormalTok{ PrePoly }\OtherTok{(}\NormalTok{ℓ ⊔ ℓ\textquotesingle{}}\OtherTok{)} \OtherTok{(}\NormalTok{κ ⊔ κ\textquotesingle{}}\OtherTok{)}
\OtherTok{(}\NormalTok{A , B}\OtherTok{)}\NormalTok{ ⊗ }\OtherTok{(}\NormalTok{C , D}\OtherTok{)} \OtherTok{=} \OtherTok{(}\NormalTok{Σ A }\OtherTok{(λ} \OtherTok{\_} \OtherTok{→}\NormalTok{ C}\OtherTok{)}\NormalTok{ , }\OtherTok{λ} \OtherTok{(}\NormalTok{a , c}\OtherTok{)} \OtherTok{→}\NormalTok{ Σ }\OtherTok{(}\NormalTok{B a}\OtherTok{)} \OtherTok{(λ} \OtherTok{\_} \OtherTok{→}\NormalTok{ D c}\OtherTok{))}
\end{Highlighting}
\end{Shaded}
